...
| O-2 | 운영 비밀번호(5자)는 설정 화면의 8자 이상 규칙(D-1, PM 결정 유지) 때문에 앱에서 되돌릴 수 없다. QA 는 README 복구 SQL 로 복원했다 | low | §5 아침 할 일 2 에서 8자 이상으로 변경 |
...
2. **비밀번호 변경**: 앱 로그인 → 설정 → 새 비밀번호(8자 이상) 2회 입력 → "비밀번호 변경". 바꾼 비밀번호를 함께 쓰는 사람에게 공유. (`.env.local` 의 `APP_OWNER_PASSWORD` 는 로그인 화면과 무관하므로 갱신은 선택.)
