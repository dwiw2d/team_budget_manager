...
- 상호는 (a) 라벨 값 → (c) 사업자번호 줄(같은 줄 → 바로 윗줄) → (b) 대표자/TEL 줄의 오른쪽 끝 토막 → (c2) 사업자번호 줄에서 위로 세 줄까지 → (d) 맨 위 네 줄 순서로 찾는다. (c) 가 (b) 보다 앞이다 — 상호가 제 줄에 따로 있는데도 사업자번호 줄 오른쪽 끝의 대표자 이름을 집어가는 영수증이 있다. (a) 의 라벨은 `상호`·`가맹점명`·`가맹점`·`매장`·`점명`·`영화관`·`주문매장`·`판매자상호`·`공급자상호`·`법인명` 이고, 자간 공백을 지운 뒤 낱말 통째로 같을 때만 라벨로 본다 — 그래야 VAN 안내 문구 `가맹점명/주소가 실제와 다른경우` 가 라벨로 읽히지 않는다. **콜론은 선택이다**(표 칸이라 `상  호   열매약국` 처럼 공백으로만 떨어진 영수증이 많다). 다만 콜론이 없으면 라벨 토막과 값 토막이 가로로 떨어져 있을 때만 인정한다. 값은 줄 끝까지가 아니라 다음 라벨이나 다음 토막 앞까지만 자른다(`상호: 두리이비인후과    대표자: 홍정주` 에서 `두리이비인후과` 만). (c) 는 `사업자번호`/`사업자등록번호` 라벨과 `\d{3}-\d{2}-\d{5}` 모양을 둘 다 본다. 번호가 상호와 같은 줄이면 **그 번호가 든 칸의 왼쪽**을 상호로 본다(`이마트 탄현점 128-85-48537 대표: 최병훈`). 번호가 제 칸의 맨 앞이면 왼쪽이 비었으므로 쓰지 않는다 — `손은주   669-56-00790` 의 대표자 이름을 집는 것을 막는다. **확신도**: 라벨로 찾았거나(a) 사업자번호 라벨이 실제로 찍힌 줄이면 확신하고, 번호 모양만 보고 찾았거나 (c2)·(d) 처럼 자리만 보고 골랐으면 값을 내되 `weak` 에 `merchant` 를 단다. 상호 거름망은 2~25자, 대괄호 없음, 글자 2자 이상, 숫자 3연속 없음(**라벨이 직접 가리킨 값은 면제** — `서울법인115`·`153구포국수(선릉역점)`), 주소·역할어·VAN사·안내 문구 아님이다. 거름망은 자간 공백을 붙인 꼴로도 한 번 더 본다(`대 표 자 명` = `대표자명`). 시/도 이름은 뒤에 행정 접미사나 공백이 올 때만 주소로 본다 — 접두어로만 거르면 `강원대학교병원`·`제주도횟집` 이 날아간다.
