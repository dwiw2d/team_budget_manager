...
**목표**: 공용 비밀번호 하나로 여러 사람이 접속하는 모바일 우선 PWA. 카드를 등록해 초기 잔액을 두고, 영수증 입력(네이버 CLOVA General OCR + 필요할 때만 Gemini 보완) 또는 수동 입력으로 결제를 기록하면 카드 잔액이 줄어든다. 언제든 접속해 총 잔액·카드별 잔액·결제 내역을 본다.
...
| 3 | 카드마다 초기 잔액(한 달 치 예산). **잔액은 계산값이 아니라 `cards.balance` 에 저장된 숫자다.** 결제를 넣으면 빠지고 취소하면 다시 더해진다 |
| 4 | 입금 없음. 매월 1일 0시(KST)에 잔액이 그 시점의 초기 잔액으로 다시 채워진다(월 초기화). **사용자가 누르는 초기화 버튼은 없다.** 스케줄러·배치도 두지 않고, 결제 트리거·앱 진입 시 RPC·뷰의 조회 시 보정 세 곳이 넘김을 맡는다 |
...
| 4-2 | 초기 잔액은 언제든 수정 가능하되 이번 달 잔액은 바뀌지 않는다. 다음 달 1일부터 적용된다 |
...
**매월 채움을 스케줄러 없이 구현한 방법**: `pg_cron` 같은 배치를 두지 않고, 잔액 기간이 지난 카드를 손대는 세 순간에 각각 넘긴다 — (1) 결제 트리거가 잔액을 조정하기 직전, (2) 앱 진입 시 `roll_over_balances()` RPC, (3) 그래도 아직 안 넘어간 카드를 위해 뷰 `card_balances` 가 조회 시 초기 잔액으로 보정해서 낸다.
...
5. **카드 `/cards`**: 카드 목록(이름, 카드번호 앞자리, 초기 잔액, 잔액). 목록 위에는 "카드 추가" 버튼 하나만 둔다. 카드 추가/수정 모달(이름, 초기 잔액, 카드번호 앞자리 선택): 초기 잔액 입력 아래에 "초기 잔액을 바꾸면 다음 달 1일부터 적용됩니다" 를 작은 글씨로 둔다. 앞자리 입력의 라벨은 "카드번호 앞 6~8자리(선택)"이고 그 아래에 "영수증은 뒤 4자리를 가리므로 앞자리로 맞춥니다" 한 줄을 둔다. 숫자 6~8자리가 아니면 저장하지 않는다. 카드 상세 시트: 이름·카드번호 앞자리·초기 잔액·잔액을 보여주고 초기 잔액 아래에 같은 안내 한 줄을 둔다. 버튼은 "수정"·"삭제" 둘뿐이다(`grid-cols-2`). **초기화 버튼은 카드별도 전체도 없다** — 잔액은 매월 1일 0시에 저절로 채워진다. 삭제는 확인창 후 실행하고 FK 오류면 안내.
...
- [ ] 초기 잔액을 고쳐도 이번 달 잔액은 그대로(안내 문구 "초기 잔액을 바꾸면 다음 달 1일부터 적용됩니다" 표시)
- [ ] 월 초기화: `cards.balance_month` 를 지난달로 만들어 두고 앱을 열면 잔액이 초기 잔액으로 채워짐(`roll_over_balances`), 결제를 넣으면 넘김 뒤 차감됨
