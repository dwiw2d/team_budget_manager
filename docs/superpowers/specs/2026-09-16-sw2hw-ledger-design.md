...
| 5-3 | 두 OCR 제공자의 무료 한도(CLOVA 월 100건, Gemini 하루 20건 — 비공식 실측에 맞춘 자체 상한, KST)는 물어볼 API 가 없어 우리가 직접 센다. 둘 다 소진이면 제공자를 부르지 않고 429 다 |
...
- 상호를 찾기 전에 **줄을 토막으로 가른다**: 한 낱말과 다음 낱말 사이가 글자 높이의 2배 넘게 벌어지면 다른 칸으로 본다. 영수증 한 줄은 `emart   이마트 신촌점 (02)-116-1219` 처럼 로고/상호/전화번호가 가로로 붙어 있어, 줄을 통째로 후보로 쓰면 길이·숫자 조건에 걸려 멀쩡한 상호까지 버려진다. 토막에서는 전화번호·사업자번호·바코드(숫자 3자리 이상인 낱말), 대괄호 말머리·마스킹, 기호뿐인 낱말을 지운다. 한글이 든 토막이 하나라도 있으면 한글 없는 토막(영문 로고, 번호 칸)은 버린다. 단 줄이 한 칸뿐이면 숫자 낱말은 지우지 않는다 — 지우면 `강남구 압구정로 165` 에서 번지만 빠져 주소가 상호로 통과한다.
- 상호는 (a) 라벨 값 → (c) 사업자번호 줄(같은 줄 → 바로 윗줄) → (b) 대표자/TEL 줄의 오른쪽 끝 토막 → (c2) 사업자번호 줄에서 위로 세 줄까지 → (d) 맨 위 네 줄 순서로 찾는다. (c) 가 (b) 보다 앞이다 — 상호가 제 줄에 따로 있는데도 사업자번호 줄 오른쪽 끝의 대표자 이름을 집어가는 영수증이 있다. (a) 의 라벨은 `상호`·`가맹점명`·`가맹점`·`매장`·`점명`·`영화관`·`주문매장`·`판매자상호`·`공급자상호`·`법인명` 이고, 자간 공백을 지운 뒤 낱말 통째로 같을 때만 라벨로 본다 — 그래야 VAN 안내 문구 `가맹점명/주소가 실제와 다른경우` 가 라벨로 읽히지 않는다. **콜론은 선택이다**(표 칸이라 `상  호   열매약국` 처럼 공백으로만 떨어진 영수증이 많다). 다만 콜론이 없으면 라벨 토막과 값 토막이 가로로 떨어져 있을 때만 인정한다. 값은 줄 끝까지가 아니라 다음 라벨이나 다음 토막 앞까지만 자른다(`상호: 두리이비인후과    대표자: 홍정주` 에서 `두리이비인후과` 만). (c) 는 `사업자번호`/`사업자등록번호` 라벨과 `\d{3}-\d{2}-\d{5}` 모양을 둘 다 본다. 번호가 상호와 같은 줄이면 **그 번호가 든 칸의 왼쪽**을 상호로 본다(`이마트 탄현점 128-85-48537 대표: 최병훈`). 번호가 제 칸의 맨 앞이면 왼쪽이 비었으므로 쓰지 않는다 — `손은주   669-56-00790` 의 대표자 이름을 집는 것을 막는다. **확신도**: 라벨로 찾았거나(a) **앵커 줄이 사업자번호 라벨과 번호만으로 이루어진 깨끗한 줄**이면 확신하고, 그 밖에는 값을 내되 `weak` 에 `merchant` 를 단다. 라벨이 있다는 사실은 그 줄이 사업자번호 줄임을 말할 뿐 윗줄이 상호임을 보장하지 않아서다 — 표 머리글(`사업자등록번호 가맹점 전화번호`)이나 상호 칸이 더 붙은 줄(`사업자등록번호 895-38-00624 상 호`) 위에는 안내문·표어가 온다. 번호 모양만 보고 찾았거나 (c2)·(d) 처럼 자리만 보고 고른 경우도 마찬가지로 확신하지 않는다. 상호 거름망은 2~25자, 대괄호 없음, 글자 2자 이상, 주소·역할어·VAN사 아님이고, 여기에 **라벨이 직접 가리킨 값은 면제하는** 셋이 더 붙는다: 숫자 3연속 없음(`서울법인115`·`153구포국수(선릉역점)`), 한글 없는 두 글자 아님(`No 001-22-33444` 의 `No`. 다만 `상호: CU` 의 `CU` 는 상호다), 인사말·안내 문구 아님. 인사말·안내 문구는 두 갈래로 본다: `…니다`·`…세요` 같은 존댓말 종결어미와 `교환`·`환불`·`적립`·`반품`, 그리고 `감사`·`고객` 처럼 상호에도 쓰이는 말(`감사식당`·`고객만족센터`)은 그 말뿐인 단독 문장일 때만 거른다. 이 셋은 앵커가 없어 자리로 추측할 때 쓰라고 만든 것이라 라벨 경로에 겹치면 `상호: 또오세요분식` 같은 멀쩡한 상호를 버린다. 거름망은 자간 공백을 붙인 꼴로도 한 번 더 본다(`대 표 자 명` = `대표자명`). 시/도 이름은 뒤에 행정 접미사나 공백이 올 때만 주소로 본다 — 접두어로만 거르면 `강원대학교병원`·`제주도횟집` 이 날아간다.
- **금액**은 라벨을 세 등급으로 나눠 **가장 높은 등급 안에서만** 다수결한다(같은 표수면 큰 값). 2등급 = 사용자가 낸 돈(`납부할금액`·`수납금액`·`받을금액`·`받은금액`·`결제금액`·`결제요금`·`결제액`), 1등급 = 어느 쪽인지 모르는 총계(`합계`·`카드매출`·`총금액`·`주문금액`·`티켓정보`·`TOTAL`), 0등급 = 사용자가 낸 돈이 아님(`총액`·`판매금액`). 등급 없이 큰 값을 고르면 진료비·약제비 서식에서 `총액`을, 할인 영수증에서 할인 전 `합계`를 집어 **실제로 낸 돈이 아닌 값이 저장돼 카드 잔액이 틀어진다**. 0등급도 후보에는 남긴다 — `판매금액` 밖에 없는 영수증이 있다. 라벨 비교는 공백을 지우고 대문자로 올린 글자로 한다(`결 제 액` = `결제액`, `Total` = `TOTAL`). 값은 줄 통째가 아니라 **라벨이 든 칸과 그 오른쪽 칸까지만** 본다(상호와 같은 토막 분리) — `총 합 계  5,600   할 인  0` 의 오른쪽 끝 `0` 을 집지 않기 위해서다. 100 미만은 금액으로 치지 않는다(`약제비총액(1+2+3)` 의 항목 번호, `합계 ①4,500 ②0 ③0` 의 빈 칸). 라벨에 `수량` 이 섞이면(`합계수량/금액  3  4,700`) 할인 전 품목 소계라 0등급으로 내린다. 라벨이 토막 경계에 걸려 쪼개지는 줄(`-  합  계   11,400`)이 있어 토막에서 아무것도 못 건지면 줄 통째로 다시 본다. 라벨이 가리킨 칸이 한 자리 숫자만 나열돼 있으면 붙여 읽는다(`합계  4 4 0 0` = 4,400). 라벨이 하나도 없을 때 쓰는 최댓값 규칙은 **천 단위 쉼표가 찍힌 숫자만** 본다 — 금액은 쉼표를 달고 나오지만 사업자번호·전화번호·연도·수량은 달지 않는다. 쉼표가 하나도 없으면 예전대로 맨숫자를 본다.
- **결제 일시**는 날짜가 든 줄을 모아 **시각이 함께 찍힌 줄 → `거래일시` 같은 라벨이 붙은 줄** 순으로 고른다. 한 장에 수납일·발행일·전표일시가 흩어져 있을 때 결제 시각까지 함께 찍힌 줄이 실제 결제 시점이다. 날짜 형식은 `2026-09-15`·`20260915`·`26/09/04` 에 더해 한글 `2021년 4월 19일` 과 공백 구분 `2022 12 30` 을 읽는다(공백 구분은 연도를 `19xx`·`20xx` 로 못 박고 맨 뒤에서만 시도해 수량 표 칸을 날짜로 읽지 않는다). 시각은 같은 줄 → 다음 줄 → 앞 줄 차례로 찾고, 그래도 없으면 **영수증 전체에 시각이 딱 하나일 때만** 그것을 쓴다(`NO:1777  14:27` 처럼 맨 아래에 따로 찍히는 판형). 시각이 여럿이면 어느 것이 결제 시각인지 알 수 없으므로 자정으로 두고 `weak` 를 단다. `오전`/`오후` 가 시각 바로 앞에 붙으면 24시간제로 바꾼다(오후 12시는 12시, 오전 12시는 0시).
- **카드번호**는 마스킹 여부·16자리로 매기던 점수가 **같을 때만** 앞 두 줄까지의 말머리로 가른다: `신용카드`·`체크카드`·`직불카드` 면 +1, `제휴`·`포인트`·`적립`·`멤버십` 이면 -1. 한 장에 카드번호가 둘일 때 결제한 카드는 `신용카드 매출전표` 아래에 찍히고 제휴할인 카드·포인트 적립 카드는 제 블록 안에 따로 찍힌다 — 후자는 그 카드로 돈을 낸 것이 아니다. 후보가 하나뿐이면 말머리와 무관하게 그대로 쓴다.
- 파서는 `weak: string[]` 로 확신 없음을 알린다: merchant가 null / merchant를 자리만 보고 고름(라벨 없는 사업자번호 모양 줄 언저리, 위로 세 줄까지 올라간 (c2), 앵커가 아예 없어 맨 위 줄에서 고른 (d)) / amount가 null / amount를 "합계·총액·결제금액" 같은 낱말 없이 "가장 큰 숫자" 규칙으로 고름 / paidAt이 null / 시각을 못 찾아 `00:00:00`으로 채움. cardNumber는 영수증에 원래 없는 경우가 많아 weak에 넣지 않는다.
...
- **503 한도 환불**(`0010_ocr_quota_refund.sql`): 503 은 모델이 일을 하나도 하지 않은 것이라 무료 한도를 쓴 적이 없다. 재시도와 모델 교체를 전부 소진하고도 503 으로만 끝나면 `ocr_quota_refund('gemini')` 로 선차감 1건을 되돌린다(`used` 는 0 아래로 내려가지 않는다). 한도 초과(429)나 네트워크 오류는 되돌리지 않는다 — 전자는 진짜로 쓴 것이고, 후자는 응답만 유실됐을 수 있어 선차감이 막아야 하는 바로 그 경우다.
...
- **무료 한도**(`0005_ocr_quota.sql`): 두 제공자 모두 남은 무료 한도를 물어볼 수 있는 API 가 없으므로 우리가 직접 센다. 한도는 CLOVA General OCR **월 100건**, Gemini **하루 20건**이고 기간은 모두 한국 시간 기준이다. Gemini 의 20건은 구글 공식 값이 아니다 — 구글이 모델별 한도 표를 공식 문서에서 내렸고 20건은 제3자 비공식 실측값이라, 우리가 그 값에 맞춰 둔 자체 상한이다(`docs/ocr/gemini.md` §3). 한도는 프로젝트의 API 키에 붙는 것이라 사용자별이 아니다(`owner_id` 없음). 한도 값은 코드가 아니라 `ocr_limits(provider, period_kind, limit_count)` 표에 있어 대시보드에서 고칠 수 있다. 사용량은 `ocr_usage(provider, period, used, exhausted_at)` 에 기간 문자열(`YYYY-MM` 또는 `YYYY-MM-DD`, KST)별로 쌓인다. 기간이 바뀌면 키가 달라져 새 행이 생기므로 따로 초기화하지 않는다. 두 표는 RLS 를 켜고 `authenticated` 에 select 만 열며, 쓰기는 `security definer` 함수 `ocr_quota_consume(provider)`·`ocr_quota_exhaust(provider)`·`ocr_quota_refund(provider)` 로만 한다. 상태 조회는 `ocr_quota()` 로, 제공자별 `{provider, period, used, limit_count, remaining, available}` 과 최상위 `available`(둘 중 하나라도 쓸 수 있으면 true)을 돌려준다.
