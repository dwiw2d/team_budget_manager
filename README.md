...
- 영수증을 촬영하면 OCR로 가맹점·금액·일시·카드번호를 읽어 폼을 채우고, 뒤 4자리가 일치하는 카드가 하나면 자동 선택합니다. OCR에 실패하면 안내 후 직접 입력할 수 있습니다.
...
3. 마이그레이션 적용(Supavisor 세션 포트 5432, 사용자는 `postgres.<POOLER_TENANT_ID>`):
   ...
   ```
4. 함수 배포: `supabase/functions/ocr`를 `docker/volumes/functions/ocr`로 복사하고, compose의 `functions` 서비스 환경에 `NAVER_OCR_INVOKE_URL`, `NAVER_OCR_SECRET`을 추가한 뒤 `docker compose restart functions`. 함수 URL은 `http://<host>:8000/functions/v1/ocr`.
