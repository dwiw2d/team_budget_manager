import { resizeToDataUrl, stripDataUrlPrefix } from "./image";
...
/** 전송용 크기. OCR 정확도를 위해 보관용(1200px·0.7)보다 크고 덜 눌러 보낸다. */
...
export async function compressImage(file: File): Promise<{ image: string; format: "jpg" }> {
  const { dataUrl } = await resizeToDataUrl(file, OCR_MAX_EDGE, OCR_QUALITY);
  return { image: stripDataUrlPrefix(dataUrl), format: "jpg" };
}
